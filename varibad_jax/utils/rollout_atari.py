...
def run_rollouts_atari(
    ...
    **kwargs,
    ...
    if "dt" in config.model.name or "icl" in config.model.name:
        rollout_fn = run_rollouts_atari_dt
    else:
        rollout_fn = run_rollouts_atari_helper
    ...
    if wandb_run is not None:
        ...
        # this is in grayscale so we select the first channel
        videos = videos[..., 0:1]
        ...
        videos = videos.repeat(3, axis=-1)  # repeat the last axis to get RGB
...
def run_rollouts_atari_dt(
    ...
    **kwargs,
    ...
    for episode in tqdm.tqdm(range(config.num_eval_rollouts)):
        ...
        env = make_atari_envs(training=False, **config.env)
        obs = env.reset()
...
def run_rollouts_atari_helper(
    rng, agent, config: ConfigDict, env, env_id: str, **kwargs
):
    logging.info("rollout atari...")
    env = make_atari_envs(training=False, **config.env)
    obs, _ = env.reset()
    ...
    while not jnp.all(dones):
        ...
        transition = Transition(
            observation=obs.transpose(0, 2, 3, 1),
            action=action,
            reward=reward,
            done=done,
        )
...
#     transitions = []
#     for episode in tqdm.tqdm(range(config.num_eval_rollouts), desc="atari rollouts"):
#         timesteps = []
#         done = False
#         timestep = env.reset()

#         while not done:
#             obs = timestep.observation / 255.0
...
#             timesteps.append(transition)
#             print(ep_lengths)

#             ep_returns = ep_returns.at[episode].set(
#                 ep_returns[episode] + timestep.reward
#             )
#             ep_lengths = ep_lengths.at[episode].set(ep_lengths[episode] + 1)

#         transitions.append(timesteps)
